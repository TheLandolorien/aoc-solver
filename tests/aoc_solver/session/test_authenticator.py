...
def test_authenticate_creates_new_session_cookie_if_missing(mock_open, mock_os, mock_importer, mock_session, provider):
...
def test_authenticate_refreshes_invalid_session_cookie(mock_open, mock_os, mock_importer, mock_session, provider):
    mock_os.path.isfile.return_value = True
    mock_importer.module_from_spec.return_value.authenticate.return_value = "foo"
...
def test_authenticate_raises_for_unattainable_session_cookie(mock_os, mock_importer):
    ...
    assert str(err.value) == "Unable to authenticate Advent of Code session with github", "should provide reason for AttributeError"
    mock_os.path.isfile.assert_called_once_with(path=authenticator.CONFIG_FILE_PATH)
