...
def test_authenticate_appends_to_existing_config(mock_browser, mock_session, mock_config_file):
    ...
    assert mock_config_file.read_text() == '{"existing_key": "existing_value", "AOC_SESSION": "foo"}', "should extend existing config"


@patch("selenium.webdriver.Firefox")
def test_authenticate_overwrite_existing_session_cookie(mock_browser, mock_session, mock_config_file):
    mock_browser.return_value.current_url = AOC_URL
    ...
    assert mock_config_file.read_text() == '{"AOC_SESSION": "bar"}', "should overwrite existing session"
